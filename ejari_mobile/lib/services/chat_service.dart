  ...
  static String generateAIResponse(String input) {
    ...
    if (input.contains('حجز') || input.contains('احجز')) {
      return 'خطوات الحجز:\n'
          ...
          '2. اضغط "احجز الآن".\n'
          '3. أكمل التحقق للهوية وادفع.\n\n'
          'هل واجهت مشكلة في حجز معيّن؟';
    }

    if (input.contains('عقد') || input.contains('قانوني')) {
      return 'العقود موثّقة إلكترونياً وتضمن حقوق الطرفين (عقد إيجار موحد).\n'
          'تصلك نسخة PDF موقعة فور الدفع.';
    }
    ...
    if (input.contains('مكان') ||
      ...
      return 'نغطي معظم مناطق القاهرة الكبرى (التجمع، زايد، المعادي، مدينة نصر).\n'
          'استخدم خريطة البحث لرؤية العقارات القريبة منك.';
    }

    if (input.contains('سيار') || input.contains('car')) {
      return 'يتوفر أسطول متنوع (سيدان، SUV، وغيرها).\n'
          'مطلوب رخصة قيادة سارية عند التسليم.';
    }

    if (input.contains('شكرا') || input.contains('تمام')) {
      return 'على الرحب. هل تحتاج مساعدة أخرى؟';
    }
